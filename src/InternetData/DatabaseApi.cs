...
/// <summary>
/// The database catalog and its downloads, reached through
/// <see cref="InternetDataClient.Database"/>.
/// </summary>
/// <remarks>
/// Access is granted by contract rather than self-serve, so every method here answers
/// <see cref="ErrorKind.Unauthorized"/> for a key without the <c>db.download</c> scope. Named
/// <c>DatabaseApi</c> rather than <c>Database</c> because that is already the shape of one database
/// family, and it is what the other InternetData SDKs call this type.
/// </remarks>
public sealed class DatabaseApi
{
        ...
        string id, DatabaseFormat format, CancellationToken cancellationToken)
    {
        ...
        return await Wire.ExecuteAsync(retries, timeout, async ct =>
        {
            ...
            response.Dispose();
            throw new ClassifiedException(new InternetDataException(
